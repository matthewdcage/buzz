...
///
/// TODO(honcho): confirm the exact response envelope against a live
/// Honcho deployment's `/openapi.json`. Honcho's MCP tool for this
/// operation is `get_peer_context`; the REST endpoint modeled below
/// (`fetch_peer_context`) is this module's best-effort guess at its
/// HTTP-API equivalent, written defensively (accepts either a
/// `representation` or `context` field, ignores unknown fields) so a
/// close-but-not-exact real shape still degrades to "no section" instead
/// of a hard error.
...
async fn fetch_peer_context(
    ...
    // TODO(honcho): confirm this endpoint path against a live API. Modeled
    // on Honcho's `get_peer_context` MCP tool; the REST API was not
    // reachable from this sandbox (connection refused on
    // http://localhost:8000) to verify against `/openapi.json`.
    let url = format!(
    ...
    let mut req = client.post(&url).header("X-Honcho-User-Name", user_name);
